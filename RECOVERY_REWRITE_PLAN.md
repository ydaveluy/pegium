...
The implementation is now aligned with the target runtime closely enough that
no numbered phase or tracked cross-phase deviation remains open. What stays in
this document now is the invariant set, the target model, and the closure
record of the rewrite.

At this stage:

- No numbered rewrite phase remains open.
- No tracked cross-phase deviation remains open.
...
- same-start continuing structural repairs prefer the stronger visible continuation before raw edit cost
...
Phase closure also requires `docs/reference/parser-recovery.md` to describe the
real runtime and avoid preserving historical family names when the shared model
is enough to explain the behavior.
