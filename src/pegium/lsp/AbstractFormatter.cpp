...
normalize_tag_line(std::string_view line,
  ...
  line = trim_horizontal(line);
  if (!options.normalizeTags || line.empty() || line.front() != '@') {
...
create_hidden_text_edits(const std::optional<CstNodeView> &previous,
                         ...
                         const FormattingContext &context) {
  const auto startLine = context.document.offsetToPosition(hidden.getBegin()).line;
  const auto previousEndsOnSameLine =
      previous.has_value() &&
      context.document.offsetToPosition(previous->getEnd()).line == startLine;

  const auto lineStartOffset =
      context.document.positionToOffset(text::Position(startLine, 0));
  const auto hiddenStartText =
      context.document.textView().substr(lineStartOffset,
  ...
  const auto normalizedHiddenText =
      ((move == nullptr ? !previousEndsOnSameLine : !move->characters.has_value()) &&
       characterIncrease != 0)
          ? reindent_hidden_text(targetText, characterIncrease, effectiveContext)
...
std::string AbstractFormatter::formatMultilineComment(
    std::string_view text, MultilineCommentFormatOptions options) {
  ...
  if (!options.forceMultiline && !hasLineBreak) {
    const auto content = normalize_tag_line(strip_comment_line_prefix(body), options);
  ...
  for (auto it = first; it != last; ++it) {
    ...
    if (options.normalizeTags && !normalized.empty() && normalized.front() == '@') {
      lines.push_back(
...
std::string AbstractFormatter::formatMultilineComment(
    const HiddenNodeFormatter &comment, MultilineCommentFormatOptions options) {
  return formatMultilineComment(comment.text(), std::move(options));
}
...
bool AbstractFormatter::isFormatRangeErrorFree(
  ...
  for (const auto &diagnostic : document.parseResult.parseDiagnostics) {
    earliestErrorLine = std::min(
        earliestErrorLine, document.offsetToPosition(diagnostic.offset).line);
  }
...
std::vector<::lsp::TextEdit> AbstractFormatter::doDocumentFormat(
  ...
  for (const auto *node : root->getAllContent()) {
    ...
    if (node == nullptr || !node->hasCstNode() ||
        !node_inside_range(document, node->getCstNode(), range)) {
      continue;
    }
    format(builder, node);
  }

  FormattingContext context{.document = document, .options = options};
