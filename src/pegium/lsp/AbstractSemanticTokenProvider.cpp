...
bool ranges_overlap(const workspace::Document &document, const ::lsp::Range &left,
  ...
  const auto leftBegin = document.positionToOffset(left.start);
  const auto leftEnd = document.positionToOffset(left.end);
  const auto rightBegin = document.positionToOffset(right.start);
  const auto rightEnd = document.positionToOffset(right.end);
...
void append_semantic_token(
  ...
  const auto startOffset = document.positionToOffset(token.range.start);
  const auto endOffset = document.positionToOffset(token.range.end);
...
AbstractSemanticTokenProvider::AbstractSemanticTokenProvider(
    ...
    : services::DefaultLanguageService(services) {
  if (languageServices.sharedServices.workspace.textDocuments != nullptr) {
    _textDocumentCloseSubscription =
        languageServices.sharedServices.workspace.textDocuments->onDidClose(
            [this](const workspace::TextDocumentEvent &event) {
...
void AbstractSemanticTokenProvider::highlightNode(
  ...
  if (document == nullptr || !node.hasCstNode()) {
    return;
  }
  highlightRange(
      to_lsp_range(*document, node.getCstNode().getBegin(),
                   node.getCstNode().getEnd()),
