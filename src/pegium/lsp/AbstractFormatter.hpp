...
///
/// Most formatters should use the helpers in `pegium::lsp::Formatting` instead
...
struct MultilineCommentFormatOptions {
  ...
  /// Normalizes tag lines such as `@param   x   value`.
  bool normalizeTags = true;
...
class AbstractFormatter : protected services::DefaultLanguageService,
                          public services::Formatter {
public:
...
protected:
  ...
  using MultilineCommentFormatOptions = pegium::lsp::MultilineCommentFormatOptions;
  using HiddenNodeFormatter = pegium::lsp::HiddenNodeFormatter;
