...
bool is_quick_fix_kind(const Kind &kind) {
  ...
  if constexpr (std::is_same_v<KindType, ::lsp::CodeActionKindEnum>) {
    ...
    const auto mapped = pegium::lsp::to_lsp_code_action_kind(
...
DefaultCodeActionProvider::extractDefaultCodeActions(
  ...
  const auto data = pegium::lsp::from_lsp_any(*diagnostic.data);
  if (!data.isObject()) {
    return std::nullopt;
  }
...
std::optional<::lsp::CodeAction> DefaultCodeActionProvider::makeDefaultCodeAction(
  ...
  textEdit.range.start = document.offsetToPosition(edit.begin);
  textEdit.range.end = document.offsetToPosition(edit.end);
...
std::optional<std::vector<::lsp::OneOf<::lsp::Command, ::lsp::CodeAction>>>
DefaultCodeActionProvider::makeCodeActionsResult(CodeActionResult actions) const {
