...
symbol_id_from_item(const ::lsp::CallHierarchyItem &item) {
...
const AstNode *resolve_call_hierarchy_target(
    const workspace::Document &document, const ::lsp::CallHierarchyItem &item,
...
AbstractCallHierarchyProvider::prepareCallHierarchy(
  ...
  if (auto item = createCallHierarchyItem(*declarations.front());
      item.has_value()) {
...
AbstractCallHierarchyProvider::incomingCalls(
  ...
  if (const auto *node =
          resolve_call_hierarchy_target(*document, params.item, references);
      node != nullptr) {
    return getIncomingCalls(*node, cancelToken);
  }
  return {};
}
...
AbstractCallHierarchyProvider::outgoingCalls(
  ...
  if (const auto *node =
          resolve_call_hierarchy_target(*document, params.item, references);
      node != nullptr) {
    return getOutgoingCalls(*node, cancelToken);
  }
  return {};
}
