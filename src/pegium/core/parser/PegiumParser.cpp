...
ParseResult PegiumParser::parse(text::TextSnapshot text,
  ...
  ParseResult result;
  const auto input = text.view();
  const TextOffset inputSize = static_cast<TextOffset>(input.size());
  ...
  {
    auto strictResult =
        detail::run_strict_parse(entryRule, skipper, text, cancelToken);
  ...
  if (!selectedAttempt.fullMatch && options.recoveryEnabled) {
    ...
    const auto initialFailureAnalysis =
        detail::analyze_failure(entryRule, skipper, text, strictSummary,
                                cancelToken);
