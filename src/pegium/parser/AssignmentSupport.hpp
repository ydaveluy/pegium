...
#include <pegium/parser/FeatureValueSupport.hpp>
...
template <auto feature> struct AssignmentReferenceSupport {
  ...
  [[nodiscard]] static std::type_index getReferenceType() noexcept {
    if constexpr (isReference) {
      return std::type_index(typeid(typename ReferenceInfo::TargetType));
    }
    return std::type_index(typeid(void));
  }

  [[nodiscard]] static bool
  acceptsReferenceTarget(const AstNode *node) noexcept {
    if constexpr (isReference) {
      return node != nullptr &&
             dynamic_cast<const typename ReferenceInfo::TargetType *>(node) !=
                 nullptr;
    }
    (void)node;
    return false;
      ...
      if constexpr (pegium::is_reference_v<OptionalValueType>) {
        if constexpr (std::same_as<RawValueType, std::string> ||
          ...
          helpers::AssignmentHelper<AttrType>{}(
              astNode, member, OptionalValueType{std::string{
