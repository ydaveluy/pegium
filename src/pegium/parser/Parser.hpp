...
struct ExpectPath {
  ...
  [[nodiscard]] const grammar::AbstractRule *contextRule() const noexcept {
    for (auto it = elements.rbegin(); it != elements.rend(); ++it) {
      if (*it != nullptr && isRuleElementKind((*it)->getKind())) {
  ...
  [[nodiscard]] const grammar::Assignment *contextAssignment() const noexcept {
    for (auto it = elements.rbegin(); it != elements.rend(); ++it) {
      if (*it != nullptr &&
          (*it)->getKind() == grammar::ElementKind::Assignment) {
...
public:
  ...
  /// Parses `document.text`, fills `document.parseResult`, and may update AST data.
  virtual void parse(workspace::Document &document,
