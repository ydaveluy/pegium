...
TEST(SyntaxTreeTest, CstNodeJsonAndStreamIncludeHiddenAndGrammarSource) {
  ...
  EXPECT_NE(json.find("\"grammarSource\": \"dummy"), std::string::npos);
...
TEST(SyntaxTreeTest, RootJsonEscapesFullTextCharacters) {
  ...
  pegium::workspace::Document document;
  document.setText(std::move(input));
