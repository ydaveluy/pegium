...
int run_language_server(::lsp::io::Stream &stream) {
  ...
  if (!requirements::services::register_language_services(services)) {
    std::cerr << "Failed to register requirements language services\n";
    return 1;
  }
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "requirements-lsp fatal error: " << error.what() << '\n';
    return 2;
  }
}
