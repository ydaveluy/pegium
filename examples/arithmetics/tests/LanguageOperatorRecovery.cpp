...
/// Phase F/E regression corpus. Pegium recovery used to commit multi-edit
...
/// shared `RecoveryKey` selects once the multi-strategy probe (Phase F/A)
/// surfaces it. These tests pin the post-F/A behaviour so the symptom
...
void expect_recovers_within(const std::string &text, std::size_t maxEdits) {
  ...
  EXPECT_LE(parsed.parseDiagnostics.size(), maxEdits)
      << "edit count regressed beyond Phase F/A bound\n"
      << parseDump;
...
TEST(ArithmeticsOperatorRecoveryTest, TrailingPlusBeforeSemicolonDeletes) {
  // `2 * c-4+;` was the F/A target case. Expected: 1 `delete +`,
  // producing `2 * (c - 4)`. Old behaviour: 5 edits fabricating
  // `c(,4)` as a function call.
  expect_recovers_within(
      "Module basicMath\n"
      "\n"
      "def c: 5;\n"
      "\n"
...
TEST(ArithmeticsOperatorRecoveryTest,
...
TEST(ArithmeticsOperatorRecoveryTest, TrailingMinusAfterPrimaryDeletes) {
  expect_recovers_within(
      "Module m\n"
      "\n"
      "def c: 5;\n"
...
TEST(ArithmeticsOperatorRecoveryTest, DoubledStarPlusOperatorDeletes) {
...
TEST(ArithmeticsOperatorRecoveryTest, DoubledMinusPlusOperatorDeletes) {
  expect_recovers_within(
      "Module m\n"
      "\n"
      "def c: 5;\n"
...
TEST(ArithmeticsOperatorRecoveryTest, DoubledStarMinusOperatorDeletes) {
  expect_recovers_within(
      "Module m\n"
      "\n"
      "def c: 5;\n"
...
TEST(ArithmeticsOperatorRecoveryTest, DoubledSlashStarOperatorDeletes) {
  expect_recovers_within(
      "Module m\n"
      "\n"
      "def c: 5;\n"
...
TEST(ArithmeticsOperatorRecoveryTest,
  ...
  // Phase H regression: `def c: a * +++ b;` followed by a statement with a
...
TEST(ArithmeticsOperatorRecoveryTest,
  ...
  // Same Phase H regression embedded in a multi-statement module: without
