...
int run_language_server(::lsp::io::Stream &stream) {
  ...
  if (!statemachine::services::register_language_services(services)) {
    std::cerr << "Failed to register statemachine language services\n";
    return 1;
  }
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "statemachine-lsp fatal error: " << error.what() << '\n';
    return 2;
  }
}
