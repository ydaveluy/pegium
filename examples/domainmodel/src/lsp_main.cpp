...
int run_language_server(::lsp::io::Stream &stream) {
  ...
  if (!domainmodel::services::register_language_services(services)) {
    std::cerr << "Failed to register domainmodel language services\n";
    return 1;
  }
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "domainmodel-lsp fatal error: " << error.what() << '\n';
    return 2;
  }
}
