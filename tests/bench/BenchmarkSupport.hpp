...
public:
  ...
  int runAll(std::string_view filter = {}) const {
    ...
    for (const auto &benchCase : _cases) {
      if (!filter.empty() &&
          benchCase.name.find(filter) == std::string::npos) {
        continue;
      }
      ++executed;
...
struct ExampleBenchSpec {
  ...
  bool (*registerLanguages)(services::SharedServices &) = nullptr;
  std::function<std::string(std::size_t targetBytes)> makeSource;
};

struct BenchmarkFixture {
  std::shared_ptr<services::SharedServices> sharedServices;
  const services::Services *services = nullptr;
...
inline std::shared_ptr<services::SharedServices> make_shared_services() {
  auto shared = std::make_shared<services::SharedServices>(
      services::SharedServices::NoDefaultsTag{});
...
create_fixture(const ExampleBenchSpec &spec, std::string source) {
  ...
  fixture->sharedServices = make_shared_services();
  ...
  const auto *coreServices =
      fixture->sharedServices->serviceRegistry->getServicesByLanguageId(
          spec.languageId);
  if (coreServices == nullptr) {
    throw std::runtime_error("No services registered for language '" +
                             spec.languageId + "'.");
  }
  fixture->services = dynamic_cast<const services::Services *>(coreServices);
...
create_changed_document(BenchmarkFixture &fixture) {
  auto textDocument = fixture.sharedServices->workspace.textDocuments->open(
      fixture.uri, fixture.languageId, fixture.source, 1);
  ...
  document->languageId = fixture.languageId;
...
measure_full_build_iteration(const std::shared_ptr<BenchmarkFixture> &fixture) {
  ...
  for (std::size_t index = 0; index < phaseStates.size(); ++index) {
    disposables.add(fixture->sharedServices->workspace.documentBuilder->onBuildPhase(
        phaseStates[index],
        [&phaseTimes, index](
  ...
  if (!ok || fixture->document == nullptr ||
  ...
  for (std::size_t index = 0; index < phaseTimes.size(); ++index) {
    if (!phaseTimes[index].has_value()) {
      throw std::runtime_error("Missing benchmark phase event for " +
                               std::string(benchmark_step_name(
                                   static_cast<BenchmarkStep>(index))) +
                               ".");
    }
  }
  ...
  for (std::size_t index = 1; index < phaseTimes.size(); ++index) {
    timings[index] =
        std::chrono::duration<double, std::milli>(*phaseTimes[index] -
                                                  *phaseTimes[index - 1])
            .count();
  }
