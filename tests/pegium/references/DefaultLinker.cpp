...
public:
  explicit StaticScopeProvider(std::vector<workspace::AstNodeDescription> entries)
...
private:
  bool visitScopeEntries(
      const ReferenceInfo &context,
      utils::function_ref<bool(const workspace::AstNodeDescription &)> visitor) const override {
    for (const auto &entry : _entries) {
...
TargetDocumentFixture make_target_document(services::SharedCoreServices &shared,
                                          std::string uri) {
...
make_reference_document(services::SharedCoreServices &shared,
  ...
  referrerPtr->node.initialize(
      *referrerPtr, refText, cst->get(0),
      assignment, linker);
...
make_multi_reference_document(services::SharedCoreServices &shared,
                              const references::Linker &linker, std::string uri,
                              std::string refText = "a") {
  auto document = std::make_shared<workspace::Document>(
      test::make_text_document(std::move(uri), "linker", refText));
...
make_attached_referrer(services::SharedCoreServices &shared,
                       const references::Linker &linker, std::string uri,
                       std::string refText = "a") {
  auto document = std::make_shared<workspace::Document>(
      test::make_text_document(std::move(uri), "linker", refText));
...
TEST(DefaultLinkerTest, ResolvesSyntheticReferenceInfoWithoutConcreteReference) {
  auto shared = test::make_empty_shared_core_services();
  pegium::services::installDefaultSharedCoreServices(*shared);
  ...
  ASSERT_TRUE(
      std::holds_alternative<std::vector<workspace::AstNodeDescription>>(candidates));
  ASSERT_EQ(std::get<std::vector<workspace::AstNodeDescription>>(candidates).size(),
            1u);
...
TEST(DefaultLinkerTest, LinkCatchesScopeProviderExceptionsWithoutThrowing) {
  ...
  auto services = test::make_uninstalled_core_services(*shared, "linker");
  pegium::services::installDefaultCoreServices(*services);
  services->references.scopeProvider = std::make_unique<ThrowingScopeProvider>();
  ...
  EXPECT_EQ(
      fixture.referrer->nodes.getErrorMessage(),
...
TEST(DefaultLinkerTest, InitializedReferenceResolvesAfterMoveConstruction) {
  ...
  auto services = test::make_uninstalled_core_services(*shared, "linker");
  pegium::services::installDefaultCoreServices(*services);
  auto fixture = make_target_document(*shared, test::make_file_uri("move.link"));
  const auto documentId = fixture.document->id;
  const auto targetPtr = fixture.target;

  services->references.scopeProvider = std::make_unique<StaticScopeProvider>(
      std::vector<workspace::AstNodeDescription>{{.name = "a",
                                                 .type = std::type_index(typeid(LinkerNode)),
                                                 .documentId = documentId,
                                                 .symbolId = fixture.document->makeSymbolId(
                                                     *targetPtr)}});
  ...
  reference.initialize(*refFixture.referrer, "a", refFixture.referrer->getCstNode(),
                       assignment, *linker);
...
TEST(DefaultLinkerTest, ReinitializingReferenceResetsCachedResolution) {
  ...
  auto services = test::make_uninstalled_core_services(*shared, "linker");
  pegium::services::installDefaultCoreServices(*services);
  auto fixture = make_target_document(*shared, test::make_file_uri("reinitialize.link"));
  const auto documentId = fixture.document->id;
  ...
  auto *root = static_cast<LinkerRoot *>(fixture.document->parseResult.value.get());
