...
TEST(AnyCharacterTest, RejectsEmptyInput) {
  auto result = dot.terminal(std::string_view{});
...
TEST(AnyCharacterTest, RejectsTruncatedAndInvalidUtf8LeadBytes) {
  ...
  auto truncatedResult = dot.terminal(truncatedView);
  ...
  auto invalidResult = dot.terminal(invalidView);
