...
///
/// `InfixRule` recovery is framed as two closed candidate families.
/// This suite covers the vocabulary and the legality predicates that
/// `InfixRule.hpp` dispatch reads:
///
///   1. `InfixCandidateFamily` enum is closed at 2 documented values.
...
TEST(InfixCandidateFamilyEnum, family_values_are_documented_and_closed) {
  for (const auto value : {InfixCandidateFamily::ContinueTail,
...
TEST(InfixCandidateFamilyLegality, default_facts_admit_no_family) {
  const InfixCandidateFamilyFacts facts;
...
// -----------------------------------------------------------------------------
// 5. Plan obligations as predicate-level invariants
// -----------------------------------------------------------------------------

TEST(InfixCandidateFamilyLegality,
     operator_never_synthesised_strict_op_required_for_continue) {
  // The operator is never synthesised by recovery: ContinueTail
  // requires a strict operator at the cursor.
  InfixCandidateFamilyFacts facts;
  facts.strictRhsPrimaryAfterOperator = true;
  EXPECT_FALSE(is_continue_tail_legal(facts));
  facts.strictOperatorAtCursor = true;
