...
enum class SmallUnsignedEnum : std::uint8_t { A = 7 };
enum class SmallSignedEnum : std::int16_t { B = -12 };
...
static_assert(SupportedRuleValueType<SmallUnsignedEnum>);
static_assert(SupportedRuleValueType<SmallSignedEnum>);
...
TEST(RuleValueTest, ConvertsEnumsToUnderlyingIntegralTypes) {
  {
    auto value = toRuleValue(SmallUnsignedEnum::A);
    ASSERT_TRUE(std::holds_alternative<std::uint8_t>(value));
    EXPECT_EQ(std::get<std::uint8_t>(value), static_cast<std::uint8_t>(7));
  }

  {
    auto value = toRuleValue(SmallSignedEnum::B);
    ASSERT_TRUE(std::holds_alternative<std::int16_t>(value));
    EXPECT_EQ(std::get<std::int16_t>(value), static_cast<std::int16_t>(-12));
  }
}
