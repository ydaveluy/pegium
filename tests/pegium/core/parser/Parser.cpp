...
struct RecursivePrintNode : pegium::AstNode {};

struct RecursivePrintGroup final : pegium::grammar::Group {
...
struct RecursivePrintRule final : pegium::grammar::ParserRule {
  const pegium::grammar::AbstractElement *child = nullptr;

  constexpr bool isNullable() const noexcept override { return false; }

  std::string_view getTypeName() const noexcept override {
    return "RecursivePrintNode";
  }
  ...
  bool rule(pegium::parser::ParseContext &) const override { return false; }

  bool rule(pegium::parser::TrackedParseContext &) const override {
    return false;
  }

  bool recover(pegium::parser::RecoveryContext &) const override {
    return false;
  }

  bool expect(pegium::parser::ExpectContext &) const override { return false; }

  void init(pegium::parser::AstReflectionInitContext &) const override {}
...
TEST(ParserTest, GrammarElementStreamPrintsRuleCallsByName) {
  ...
  rule.child = std::addressof(group);
  group.child = std::addressof(rule);

  std::ostringstream stream;
  stream << static_cast<const pegium::grammar::AbstractElement &>(rule);

  EXPECT_EQ(stream.str(), "Recursive returns RecursivePrintNode: (Recursive);");
}

TEST(ParserTest, GrammarElementPrintPrintsRuleCallsByName) {
  RecursivePrintRule rule;
  RecursivePrintGroup group;
  rule.child = std::addressof(group);
  group.child = std::addressof(rule);
