...
public:
  ...
  RuleParser(const pegium::CoreServices &services,
             const grammar::ParserRule &entryRule, parser::Skipper skipper,
             parser::ParseOptions options = {}) noexcept
      : PegiumParser(services), _entryRule(entryRule),
        _skipper(std::move(skipper)), _options(options) {}
...
inline void parse_rule(const grammar::ParserRule &entryRule,
  ...
  apply_parse_result(document, parser.parse(document.textDocument().getText()));
}
